(function ($) {
	...
	// notification dropdown triger js
	$('.notification-triger').on('click', function(event){
		event.stopPropagation();
	...
	$('body').on('click', function(e){
		var $target = e.target;
		if (!$($target).is('.notification-dropdown') && !$($target).parents().is('.notification-dropdown')) {
			$(".notification-dropdown").slideUp("slow");
		}
	});

	// messages dropdown triger js
	$('.messages-triger').on('click', function(event){
		event.stopPropagation();
        $(".messages-dropdown").slideToggle();
	})

	//Close When Click Outside
	$('body').on('click', function(e){
		var $target = e.target;
		if (!$($target).is('.messages-dropdown') && !$($target).parents().is('.messages-dropdown')) {
			$(".messages-dropdown").slideUp("slow");
		}
	});
